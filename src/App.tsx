...
function App(props: AppProps) {
  ...
  const onLeaveOrJoinSession = useCallback(
    async (isOneToOne: boolean = false) => {
      if (status === "closed") {
        setIsLoading(true);
        await zmClient.join(topic, signature, name, password);
        setIsLoading(false);
  ...
  const joinBreakRoomOrMainRoom = useCallback(
    async (isOneToOne: boolean = false) => {
      if (isOneToOne) {
        const signature = generateVideoToken(
          sdkKey,
          "rEYuwctq9fcPjzK99FE5fwbeevxolxdYTHCA",
          topic + "-one-to-one",
          password,
          "",
          ""
        ...
        await zmClient.leave();
        setTimeout(async () => {
          await zmClient.join(topic + "-one-to-one", signature, name, password);
        }, 2000);
        console.log("join break room");
      } else {
        await zmClient.leave();
        setTimeout(async () => {
          await zmClient.join(topic, signature, name, password);
        }, 2000);
        console.log("join main room");
      }
    },
    [zmClient, status, topic, signature, name, password]
  );
                  ...
                  <Switch>
                    ...
                    <Route path="/video">
                      {isSupportGalleryView ? (
                        <Video
                          joinBreakRoomOrMainRoom={joinBreakRoomOrMainRoom}
                        />
                      ) : (
                        <VideoSingle
                          joinBreakRoomOrMainRoom={joinBreakRoomOrMainRoom}
